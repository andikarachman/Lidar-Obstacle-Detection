...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    if(inliers->indices.size() == 0) {
        std::cout << "Could not estimate a planar model for this dataset" << std::endl;
    }
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::RansacPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold) {
    ...
    while (maxIterations--) {
        ...
        const float x1 = cloud->points[*iter].x;
        const float y1 = cloud->points[*iter].y;
        const float z1 = cloud->points[*iter].z;

        iter++;

        const float x2 = cloud->points[*iter].x;
        const float y2 = cloud->points[*iter].y;
        const float z2 = cloud->points[*iter].z;

        iter++;

        const float x3 = cloud->points[*iter].x;
        const float y3 = cloud->points[*iter].y;
        const float z3 = cloud->points[*iter].z;
        ...
        for (int index = 0; index < cloud->points.size(); index++) {
            ...
            const float x4 = point.x;
            const float y4 = point.y;
            const float z4 = point.z;

            const float dist = fabs(a * x4 + b * y4 + c * z4 + d) / sqrt(a * a + b * b + c * c);
...
template<typename PointT>
void ProcessPointClouds<PointT>::Proximity(int indice, typename pcl::PointCloud<PointT>::Ptr cloud, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol) {
...
// Euclidean cluster implementation
template<typename PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::EuclideanCluster(typename pcl::PointCloud<PointT>::Ptr cloud, KdTree* tree, float distanceTol, int minSize, int maxSize) {
    ...
    while(i < cloud->points.size()) {
        ...
        Proximity(i, cloud, clusterIdx, processed, tree, distanceTol);
        if (clusterIdx.size() >= minSize &&  clusterIdx.size() <= maxSize) {
            for (int i = 0; i < clusterIdx.size(); i++) {
                PointT point;
                point = cloud->points[clusterIdx[i]];
