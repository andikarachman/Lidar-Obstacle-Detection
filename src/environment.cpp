...
// void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer) {
    ...
    // pcl::PointCloud<pcl::PointXYZI>::Ptr filteredCloud = pointProcessorI->FilterCloud(inputCloud, 0.1f, Eigen::Vector4f(-(X / 2), -Y, -Z, 1), Eigen::Vector4f(X, Y, Z, 1));
    inputCloud = pointProcessorI.FilterCloud(inputCloud, 0.25f, Eigen::Vector4f(-(X / 2), -Y, -Z, 1), Eigen::Vector4f(X, Y, Z, 1));
    ...
    // Cluster cloud points
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI.EuclideanCluster(segmentCloud.first, tree, 0.35, 30, 1775);
